...
type TrackReceiver interface {
	...
	DeleteDownTrack(peerID types.ParticipantID)
...
type WebRTCReceiver struct {
	...
	index       map[types.ParticipantID]int
	free        map[int]struct{}
	numProcs    int
	lbThreshold int
}
...
func NewWebRTCReceiver(receiver *webrtc.RTPReceiver, track *webrtc.TrackRemote, pid types.ParticipantID, opts ...ReceiverOpts) *WebRTCReceiver {
	w := &WebRTCReceiver{
		peerID:   pid,
		receiver: receiver,
		trackID:  types.TrackID(track.ID()),
		...
		index:            make(map[types.ParticipantID]int),
...
func (w *WebRTCReceiver) SetTrackMeta(trackID types.TrackID, streamID string) {
	w.streamID = streamID
	w.trackID = trackID
}
...
func (w *WebRTCReceiver) TrackID() types.TrackID {
	return w.trackID
}
...
func (w *WebRTCReceiver) DeleteDownTrack(peerID types.ParticipantID) {
	if w.closed.get() {
		return
	}
...
func (w *WebRTCReceiver) closeTracks() {
	...
	w.index = make(map[types.ParticipantID]int)
